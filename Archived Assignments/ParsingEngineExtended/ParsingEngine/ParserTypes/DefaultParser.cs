...
namespace ParsingEngine.ParserTypes
{
    abstract class DefaultParser
    {
        ...
        public virtual void ReadFile(IParsable passedFile)
        {
            DelimitedFileObject currentFile = (DelimitedFileObject)passedFile;
            ...
            using (StreamReader sr = new StreamReader(currentFile.Path))
            {
                ...
                {
                    values = currentLine.Split(currentFile.Delimiter).ToList();
                    Items.Add(values);
        ...
        public void WriteFile(string path, List<List<string>> items)
        {
            ...
            using (FileStream fs = File.Create(newPath))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    ...
                    {
                        sw.Write($"Line#{x + 1} :");
                        for (int i = 0; i < items[x].Count; i++)
                        {
                            sw.Write($"Field#{i + 1}={items[x][i]} ");
                            if (i != items[x].Count - 1)
                            {
                                sw.Write("==> ");
                            }
                        }
